...
public class LocationController {
    ...
    @GetMapping
    public ResponseEntity<List<LocationResponse>> getAllOngs() {
        return ResponseEntity.ok().body(service.findAll());
    }

    @GetMapping("/{id}")
    public ResponseEntity<LocationResponse> getOng(@Valid @PathVariable Long id) {
        return ResponseEntity.ok().body(service.findById(id));
    }

    @PutMapping("/{id}")
    public ResponseEntity<LocationResponse> updateOng(@Valid  @PathVariable Long id, @RequestBody @Valid LocationRequest request) {
        var response = service.update(id, request);
        return ResponseEntity.ok(response);
    }

    @PostMapping
    public ResponseEntity<LocationResponse> saveOng(@Valid  @RequestBody LocationRequest request) {
